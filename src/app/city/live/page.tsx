...
export default function LivingCityPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (progressRes.status === 401) {
          router.replace("/auth?next=/city/live");
          return;
        }
  ...
  return (
    <div className="tm-shell space-y-6 pb-10">
      ...
      <div className="text-center">
        <Link href="/city" className="text-sm text-[#7d7362] hover:text-[#2f2619]">
          ← العودة لمدينة المعنى الكلاسيكية
        </Link>
      </div>
    </div>
  );
