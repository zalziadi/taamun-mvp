...
function SubscribeButton({ tierId, highlight }: { tierId: TierDef["tierId"]; highlight?: boolean }) {
  ...
  return (
        <div className="mt-6 space-y-2.5">
          ...
          {err ? <p className="text-center text-xs text-[#e8e1d9]/75">{err}</p>p> : null}
        </div>div>
      );
}
          ...
          <div dir="rtl" className="min-h-screen bg-[#15130f] px-4 pb-16 pt-6 text-[#e8e1d9]">
                <div className="mx-auto w-full max-w-6xl space-y-8">
                        <section className="rounded-3xl border border-white/10 bg-[#2b2824] p-7 sm:p-8">
                                  <p className="text-xs tracking-[0.18em] text-[#c9b88a]">PRICING</p>p>
                                  <h2 className="mt-2 font-[var(--font-amiri)] text-4xl text-[#e8e1d9]">الأسعار والاشتراك</h2>h2>
                        ...
                        <article key={tier.tierId} className="relative rounded-3xl border border-white/10 bg-[#2b2824] p-6">
                          {tier.badge ? (
                                      ...
                                      <h3 className="font-[var(--font-amiri)] text-2xl text-[#e8e1d9]">{tier.name}</h3>h3>
                                      <p className="mt-1 text-xs text-[#c9b88a]">{tier.note}</p>p>
                                      <p className="mt-4 text-3xl font-bold text-[#e8e1d9]">
                                        {tier.price} <span className="text-base font-normal">ر.س</span>span>
                                      </p>p>
                                      <p className="text-xs text-[#c9b88a]">{tier.period}</p>p>
